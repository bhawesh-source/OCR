...
# ---------- Extraction - Begin ----------
def extractText(char):
    util.displayImage(char, "Text Extraction")
    # Gray Scale Conversion
    gray = util.toGray(char)
    # Thresholding
    ...
    util.displayImage(dilate)

    cntrs, hier = cv.findContours(thresh, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)

    image_rect = char.copy()
    for cnt in cntrs:
        x, y, w, h = cv.boundingRect(cnt)
        cv.rectangle(image_rect, (x,y), (x+w, y+h), (0,255,0), 2)
        # cv.drawContours(image_rect, cnt, 0, (0,255,0), 2)
    util.displayImage(image_rect)
...
def main(image):
    ...
    # for char in chars:
    #     util.displayImage(char)

    # Clearing Contents of directory
    util.clearDir(CHAR_DIR)
    # Saving all characters
    for i,c in enumerate(chars):
        cv.imwrite(CHAR_DIR + str(i) + ".png", c)
...
if __name__ == "__main__":
    ...
    # Calling main saves the image as png in the character dir mentioned in this module
    main(image)
    #
